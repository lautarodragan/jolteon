...
use mpris_server::{
    LoopStatus, Metadata, PlaybackRate, PlaybackStatus, PlayerInterface, Property, RootInterface, Server, Time, TrackId, Volume,
    zbus,
};
...
impl RootInterface for MprisState {
    ...
    async fn set_fullscreen(&self, fullscreen: bool) -> zbus::Result<()> { Ok(()) }
    ...
    async fn identity(&self) -> zbus::fdo::Result<String> { Ok("Jolteon".to_string()) }

    async fn desktop_entry(&self) -> zbus::fdo::Result<String> { Ok("".to_string()) }

    async fn supported_uri_schemes(&self) -> zbus::fdo::Result<Vec<String>> { Ok(vec!["file".to_string()]) }
...
impl PlayerInterface for MprisState {
    ...
    async fn previous(&self) -> zbus::fdo::Result<()> { Ok(()) }
    ...
    async fn seek(&self, offset: Time) -> zbus::fdo::Result<()> { Ok(()) }

    async fn set_position(&self, track_id: TrackId, position: Time) -> zbus::fdo::Result<()> { Ok(()) }

    async fn open_uri(&self, uri: String) -> zbus::fdo::Result<()> { Ok(()) }

    async fn playback_status(&self) -> zbus::fdo::Result<PlaybackStatus> { Ok(PlaybackStatus::Stopped) }

    async fn loop_status(&self) -> zbus::fdo::Result<LoopStatus> { Ok(LoopStatus::None) }

    async fn set_loop_status(&self, loop_status: LoopStatus) -> zbus::Result<()> { Ok(()) }

    async fn rate(&self) -> zbus::fdo::Result<PlaybackRate> { Ok(1.0) }

    async fn set_rate(&self, rate: PlaybackRate) -> zbus::Result<()> { Ok(()) }

    async fn shuffle(&self) -> zbus::fdo::Result<bool> { Ok(true) }

    async fn set_shuffle(&self, shuffle: bool) -> zbus::Result<()> { Ok(()) }
    ...
    async fn volume(&self) -> zbus::fdo::Result<Volume> { Ok(1.0) }

    async fn set_volume(&self, volume: Volume) -> zbus::Result<()> { Ok(()) }

    async fn position(&self) -> zbus::fdo::Result<Time> { Ok(Time::ZERO) }

    async fn minimum_rate(&self) -> zbus::fdo::Result<PlaybackRate> { Ok(1.0) }

    async fn maximum_rate(&self) -> zbus::fdo::Result<PlaybackRate> { Ok(1.0) }

    async fn can_go_next(&self) -> zbus::fdo::Result<bool> { Ok(false) }

    async fn can_go_previous(&self) -> zbus::fdo::Result<bool> { Ok(false) }

    async fn can_play(&self) -> zbus::fdo::Result<bool> { Ok(false) }

    async fn can_pause(&self) -> zbus::fdo::Result<bool> { Ok(false) }

    async fn can_seek(&self) -> zbus::fdo::Result<bool> { Ok(false) }
